...
export const useRevisionStore = create<RevisionState>()(
  persist(
    (set, get) => ({
      ...
      initializeData: (data: ProgramProposalResponse) => {
        ...
        set({
          ...
          ...transformedData,
          modifiedSections: new Set<RevisionSection>(),
          // Reset UI state when initializing new data
          currentStep: 0,
          isRevising: false,
          isReviewing: false,
        });
      ...
      removePEO: (id) => {
        set((state) => {
          ...
          if (
            state.peo_mission_mappings.length !==
            updatedPEOMissionMappings.length
          ) {
        ...
        set((state) => {
          ...
          if (existingMapping) {
            ...
            updatedMappings = state.ga_peo_mappings.filter(
              (mapping) =>
                !(mapping.ga_id === ga_id && mapping.peo_id === peo_id)
            );
          } else {
            // Add the mapping if it doesn't exist
      ...
      removePO: (id) => {
        set((state) => {
          ...
          if (
            state.course_po_mappings.length !== updatedCoursePOMappings.length
          ) {
        ...
        set((state) => {
          ...
          if (existingMapping) {
            ...
            updatedMappings = state.po_peo_mappings.filter(
              (mapping) =>
                !(mapping.po_id === po_id && mapping.peo_id === peo_id)
            );
          } else {
            // Add the mapping if it doesn't exist
      ...
      updateCourseCategory: (
        ...
        set((state) => {
          ...
          const updatedCurriculumCourses = state.curriculum_courses.map(
            (course) =>
        ...
        set((state) => {
          ...
          switch (section) {
            ...
            case "peo_mission_mappings":
              newState.peo_mission_mappings =
                transformedData.peo_mission_mappings;
              break;
      ...
      submitRevisions: async () => {
        ...
        return new Promise<boolean>((resolve) => {
          setTimeout(() => {
            // Clear the localStorage when successful
    ...
    {
      name: "program-revision-storage", // unique name for localStorage key
      ...
      partialize: (state) => ({
        ...
        peos: state.peos,
        peo_mission_mappings: state.peo_mission_mappings,
        ga_peo_mappings: state.ga_peo_mappings,
        pos: state.pos,
        po_peo_mappings: state.po_peo_mappings,
        po_ga_mappings: state.po_ga_mappings,
        curriculum: state.curriculum,
        course_categories: state.course_categories,
        curriculum_courses: state.curriculum_courses,
        course_po_mappings: state.course_po_mappings,

        // Include tracking of modifications
        modifiedSections: Array.from(state.modifiedSections),
      }),
      // Transform state when hydrating from storage (needed for Set)
      onRehydrateStorage: () => (state) => {
        // Convert modifiedSections array back to Set when rehydrating

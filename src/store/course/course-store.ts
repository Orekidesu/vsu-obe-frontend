...
import { persist, createJSONStorage } from "zustand/middleware"; // Add these imports
...
export const createCourseDetailsStore = (courseId: string) => {
  const useStore = create<CourseDetailsState>()(
    persist(
      (set, get) => ({
        // Initial state
        courseId: "",
          ...
          set((state) => {
            const newId =
              state.courseOutcomes.length > 0
                ? Math.max(
                    ...state.courseOutcomes.map((outcome) => outcome.id)
                  ) + 1
                : 1;
            return {
              courseOutcomes: [
        ...
        updateCourseOutcomePO: (
          ...
          set((state) => {
            ...
            if (contributionLevel === null) {
              ...
              if (
                po &&
                po.availableContributionLevels?.includes(contributionLevel)
              ) {
                if (existingMappingIndex >= 0) {
                  // Update existing mapping
                  updatedMappings[existingMappingIndex] = {
        ...
        removeAssessmentTask: (id) =>
          set((state) => ({
            assessmentTasks: state.assessmentTasks.filter(
              (task) => task.id !== id
            ),
          })),
        ...
        resetStore: () => {
          // Clear localStorage for this specific course
          localStorage.removeItem(`course-details-${courseId}`);

          // Reset state to initial values
          set({
            courseId: courseId, // Keep the courseId
            courseCode: "",
            courseTitle: "",
            courseOutcomes: [],
            programOutcomes: [],
            ...
            coPoMappings: [],
            assessmentTasks: [],
            // Keep default teaching methods and learning resources
            teachingMethods: get().teachingMethods,
            learningResources: get().learningResources,
            coTLMappings: [],
            currentStep: 1,
          });
        },
      ...
      {
        name: `course-details-${courseId}`, // unique name for localStorage key
        ...
        partialize: (state) => ({
          ...
          coAbcdMappings: state.coAbcdMappings,
          coCpaMappings: state.coCpaMappings,
          coPoMappings: state.coPoMappings,

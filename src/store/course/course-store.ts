  ...
  persist(
    (set, get) => ({
        ...
        set((state) => {
          const newId =
            state.courseOutcomes.length > 0
              ? Math.max(...state.courseOutcomes.map((outcome) => outcome.id)) +
                1
              : 1;
          return {
            courseOutcomes: [
      ...
      resetStore: () => {
        ...
        localStorage.removeItem("course-details-storage");
    ...
    {
      name: "course-details-storage", // unique name for localStorage key
